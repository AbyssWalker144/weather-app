...
(function getWeatherData () {
    ...
    fetch(`http://api.openweathermap.org/data/2.5/forecast?lat=50.45&lon=30.52&appid=${API_KEY}&units=metric`)
    .then(res2 => res2.json())
    .then(data2 => {
        console.log(data2);
        temperature.text(Math.round(data2.list[0].main.temp));
        
        mainImg.attr('src', `http://openweathermap.org/img/wn/${data2.list[0].weather[0].icon}@4x.png`)
